##  This is python reference code for the opencl kernel hbk_clist_propagator
...
def hbk_clist_propagator_ref( 

    tx_array_element_descriptor,
    sampling_point_list,
    tx_count,medium_wavenumber,
    recp_transducer_nan_size
    ...
    '''
    ---------------------------------------------
    hbk_clist_propagator_ref(tx_array_element_descriptor,sampling_point_list,tx_count,medium_wavenumber,recp_transducer_nan_size)
    ---------------------------------------------
    This method samples the propagated acoustic field at a set of points describing
    a surface or volume. 
    ...
    recp_transducer_nan_size : float
                    Reciprocal of the wavelength - this variable prevents sampling 
                    of the acoustic field inside a transducer.
    '''
    ...
    # For each point in the sampling point list calculate the contribution from 
    # each transducer. 
    
    for point in sampling_point_list:

        pixel_x_coordinate = point[0]
        pixel_y_coordinate = point[1]
        pixel_z_coordinate = point[2]
